...
function kadence_display_staff_breadcrumbs() {
  ...
  if($virtue_premium['show_breadcrumbs_staff'] == 1 ) {$showbreadcrumbs = true;} else { $showbreadcrumbs = false;}
} else {$showbreadcrumbs = true;}
  return $showbreadcrumbs;
}
...
function kadence_breadcrumbs() {
  ...
  if(!empty($virtue_premium['home_breadcrumb_text'])) {$home = $virtue_premium['home_breadcrumb_text'];} else {$home = __('Home', 'virtue');}
        ...
        if ( get_post_type() != 'post' ) {
          ...
          if($post_type == "staff") {
              if( !empty($virtue_premium['staff_link']) ) { 
                    $parentpagelink = get_page_link($virtue_premium['staff_link']); $parenttitle = get_the_title($virtue_premium['staff_link']);
                    echo '<span itemscope itemtype="http://data-vocabulary.org/Breadcrumb"><a itemprop="url" href="'.$parentpagelink. '"><span itemprop="title">' . $parenttitle . '</span></a></span> ' . $delimiter . ' ';
                  } 
